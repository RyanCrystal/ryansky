...
var convertPackage = function (stat, type, callback) {
	...
	exec(encoder[type] + " '" + src + "'", function (err) { // run encoder
		...
		fs.readFile(format, function (err, buffer) { 
      console.log(err);
      console.log(buffer)// read encoded file
...
MIDI.Soundfont["' + instrumentName + '"] = ';
	...
	///
	var buf = new Buffer.from(js, "utf-8"); // Choose encoding for the string.
